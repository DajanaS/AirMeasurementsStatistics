...
## 1. Description
Our project Air Measurement Statistics is intended for the Microcontroller PIC16F887 (frequency 8 Mhz). Once it is turned on, the user sees the menu on the `GLCD` screen (Graphic LCD). The 4 different options from the menu can be chosen by using KeyPad. Each is explained in the following paragraph.
## 2. Guide
...
On this screen the current temperature that the temperature sensor will measure will be shown. Lights from portC will turn on depending of how high is the temperature value. The range of all possible temperature values is mapped in 8 ranges, one range for each light, respectively.
