...
## 3. Implementation
The main controlling component is the KeyPad Menu. Control transfers to other components based on the pressed button with call to the appropriate function: 
* measureTemperature() - this function reads current temperature from the DS1820 temperature sensor. The obtained value is unsigned integer. To be able to do manipulations with this value, it needs to be converted into signed integer. Afterwards, this value is displayed on GLCD and its level is displayed on the LEDs of portC. The range from the temperature obtained from the sensor is from -55<sup>o</sup>C to 125<sup>o</sup>C. This range is divided into eight parts, each representing one of the eight LEDs. For example, if temperature is 32<sup>o</sup>C four, lights will be ON.
